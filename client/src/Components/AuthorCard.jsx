...
function AuthorCard({ userData, className, viewProfileBtn }) {
  ...
  return (
    <div
      className={`relative mx-2 sm:mx-0 w-[10rem] sm:w-[12rem] border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm bg-white dark:bg-transparent hover:shadow-lg transition-all transform hover:-translate-y-1 hover:scale-[1.02] duration-300 ${className}`}
    >
      {/* Profile Image */}
      ...
      <div className="text-center my-3 px-3">
        <h3 className="text-sm font-semibold text-gray-800 dark:text-white truncate">{username}</h3>
      </div>
      ...
      <div className="mb-4 px-4">
        ...
        <Link to={`/conversations/${userId}`}>
          <button className="w-full py-1.5 text-xs font-medium flex items-center justify-center gap-1 border border-gray-600 text-white hover:bg-blue-50 dark:hover:bg-gray-800 rounded-full transition duration-300">
